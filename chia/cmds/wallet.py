...
def clawback(
    wallet_rpc_port: Optional[int], id: int, fingerprint: int, tx_ids: str, fee: uint64
) -> None:  # pragma: no cover
    from .wallet_funcs import spend_clawback

    asyncio.run(spend_clawback(wallet_rpc_port=wallet_rpc_port, fp=fingerprint, fee=fee, tx_ids_str=tx_ids))
...
def cancel_offer_cmd(
    ...
    asyncio.run(cancel_offer(wallet_rpc_port, fingerprint, fee, id, insecure))
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which key to use", type=int)
@click.option("-i", "--nft_id", help="NFT ID you want to use for signing", type=CliAddress, required=True)
@click.option("-m", "--hex_message", help="The hex message you want to sign", type=str, required=True)
...
# TODO: Change RPC to use puzzlehash instead of address
@click.option("-ta", "--target-address", help="Target recipient wallet address", type=CliAddress, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XCH.",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which key to use", type=int)
@click.option("-d", "--did", help="The DID of the VC's proof provider", type=CliAddress, required=True)
@click.option(
...
def revoke_vc_cmd(
    ...
    asyncio.run(revoke_vc(wallet_rpc_port, fingerprint, vc_id, parent_coin_id, fee, reuse_puzhash))
